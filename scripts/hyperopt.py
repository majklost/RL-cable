...
def objectiveCableMoving(trial):
    TIMESTEPS = 700000
    ...
    prune_clb = PruneCallback(trial=trial)
    eval_clb = EvalCallback(eval_env, best_model_save_path=PATH /
                            "best_model", eval_freq=20000, callback_on_new_best=save_norm_clb, callback_after_eval=prune_clb, n_eval_episodes=20)
    args = give_args(trial)
    model = PPO("MlpPolicy", env, verbose=0, device='cpu', **args)
    model.learn(total_timesteps=TIMESTEPS, callback=[save_norm_clb, eval_clb])
    rew, std = evaluate_policy(model, eval_env, n_eval_episodes=20)
...
def objectiveCableLocalNeighbours(trial):
    TIMESTEPS = 1000000
    env_name = CableEmptyV0LocalNeighbour.__name__
    kwargs = dict(env_kwargs=dict())
    maker = single_env_maker(CableEmptyV0LocalNeighbour, wrappers=[TimeLimit, Monitor], wrappers_args=[
...
def objectiveRadiusObsVelStronger(trial):
    TIMESTEPS = 1000000
    env_name = CableRadiusObsVelStronger.__name__
    kwargs = dict(env_kwargs=dict())
    maker = single_env_maker(CableRadiusObsVelStronger, wrappers=[TimeLimit, Monitor], wrappers_args=[
        {'max_episode_steps': 1000}, {}], render_mode='human', **kwargs["env_kwargs"])
    n_envs = trial.suggest_categorical("n_envs", [4, 16, 32])
    env = create_multi_env(maker, n_envs, normalize=True)
    eval_env = create_multi_env(maker, 1, normalize=True)
...
if __name__ == "__main__":
    ...
    # study = optuna.load_study(
    #     study_name="CableMoving", storage=f"sqlite:///{PATH / 'CableMoving.db'}")
    # print("Starting optimization")
    # study.optimize(objectiveCableMoving, n_trials=80,
    #                timeout=None, show_progress_bar=True, gc_after_trial=True)

    # study = optuna.load_study(
    #     study_name="CableLocalNeighbours", storage=f"sqlite:///{PATH / 'CableLocalNeighbours.db'}")
    # print("Starting optimization")
    # study.optimize(objectiveCableLocalNeighbours, n_trials=100,
    #                timeout=None, show_progress_bar=True, gc_after_trial=True)

    study = optuna.load_study(
        study_name="RadiusObsVel", storage=f"sqlite:///{PATH / 'RadiusObsVel.db'}")
    print("Starting optimization")
    study.optimize(objectiveRadiusObsVelStronger, n_trials=100,
